...
実行するだけで:
  ...
  6. closing_watch.py      14:55〜15:15 引け前スキャン・発注（戦略B）
...
def main():
    ...
    rc = run_step("【Step 1/6】download_db.py（S3から最新DBをダウンロード）",
                  [py, "download_db.py", "--db-only"], cwd)
    if rc != 0:
        print("⚠️  DBダウンロード失敗。ローカルDBで続行します。")

    # ── Step 2: 電話認証 + ログイン ───────────────────────
    rc = run_step("【Step 2/6】Tachibana 電話認証 + ログイン",
    ...
    rc = run_step(f"【Step 3/6】scan_daily.py --date {prev_fmt}（前営業日データ取得）",
    ...
    rc = run_step("【Step 4/6】scan_morning.py（地合い予測・候補選定）",
    ...
    rc = run_step("【Step 5/6】market_watch.py（9:00〜9:30 リアルタイム監視・戦略A発注）",
                  [py, "market_watch.py"], cwd)
    # market_watch は時間切れでも rc=0 なので中断しない

    # ── Step 6: closing_watch.py（14:55〜15:15）──────────
    closing_cmd = [py, "closing_watch.py"]
    if args.manual:
        closing_cmd.append("--manual")
    run_step("【Step 6/6】closing_watch.py（14:55〜 引け前スキャン・戦略B発注）",
             closing_cmd, cwd)
