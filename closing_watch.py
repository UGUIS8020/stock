...
DEFAULT_SHARES   = 300      # 2026-08-01: 100→200（2026-08-06: 300へ一時変更したが資金所要額を
                            ...
                            # 平均+1.97%と良好なため300へ再変更したが、2026-08-13:
                            # MIN_PRICE導入で対象銘柄の平均株価が上がった結果、300株×上限5でも
                            # 全戦略合計が資金(1,000万円)を大幅超過(約1,881万円)すると判明した
                            # ため200株へ差し戻し。金額ベースの分岐は廃止済みのため常に固定株数。
                            # 2026-08-14: 再び300株へ変更。200株据え置きだとSTRONG日は既に
                            # 資金超過気味(約46%超)・NORMAL日は約28%の余裕があったが、300株では
                            # NORMAL日もAN込みで超過(約1,045万円)と判明。ただし資金不足時の
                            # 発注は「エラーで却下→次候補へ」というクリーンな失敗で実害がない
                            # ことを確認済みで、「全戦略が同時に最悪ケースで重なるのは稀」という
                            # 従来からの考え方でユーザーが超過を容認し、300株に変更。
