...
RB_MIN       = 3      # RBスコア最小値（2026-05-26 GA最適化時は4。2026-09-07、直近(7-8月)の
                       # 日足再検証でRB=3×連続下落OKを追加検証した結果、現行(RB>=4)と同等以上
                       # （差分40件: 勝率72.5%・平均+1.97% / 現行120件: 勝率70.0%・平均+1.80%、
                       # 8月分のみでも17件・勝率70.6%）だったため3に緩和。候補数120→160件(+33%)。
                       # 終値ベースの簡易シミュレーションでの検証であること、差分40件中10件が
                       # 2026-07-29の1日に集中している点は留意（地合いはNORMAL、特殊要因ではなさそう）。
                       # 連続下落条件(CD_MIN)を外すと明確に悪化するためこちらは維持。
...
DEFAULT_SHARES   = 300      # ¥5,000未満の株の発注株数（基準サイズ）。これまで200↔300を
                            # 複数回往復（詳細はコミット履歴参照）。2026-08-17: 資金1,300万円
                            # 到達を受け300へ再変更。2026-09-05: 価格帯別に 300/200/100株の
                            # 3段へ（calc_shares 参照）。
...
MAX_PRICE_B = 20_000  # 2026-09-03新設(¥10,000)→2026-09-05に¥20,000へ引き上げ（ユーザー判断・
                    # 直近の戦略B成績が良好だったため）。戦略Bには候補の価格上限が無かったため
                    # ファストリ68,000円→100株680万円 が発生したのが導入の経緯。
                    # 候補フィルタで MIN_PRICE(¥1,000) ≤ price < MAX_PRICE_B(¥20,000) に絞る。
                    # 戦略A(MAX_PRICE_A=10,000)・AS より上限は緩い。
...
def calc_shares(price):
    """価格帯別の発注株数を返す（候補は ¥1,000 ≤ price < ¥20,000 に絞り済み）。
      ¥1,000〜5,000  : 300株（建玉 ¥300k〜1.5M）
      ¥5,000〜10,000 : 200株（建玉 ¥1.0M〜2.0M）
      ¥10,000〜20,000: 100株（建玉 ¥1.0M〜2.0M）
    ¥5,000以上はどの帯でも建玉が概ね ¥1.0〜2.0M に収まる。
    2026-09-05変更: 旧ロジックは HIGH_PRICE_CAP_AMOUNT(¥100万) で株数を金額縮小して
    いたが、¥10,000超では100株でも¥100万超で floor(max(lots,1)) が常に発動し
    実質¥1.0〜2.0M だった。同じ結果を3段固定で素直に表現し、¥5,000〜10,000帯の
    サン電子型の過大建玉(¥2.29M/300株)も回避する。"""
    if not price or price < 5_000:
        return DEFAULT_SHARES        # 300
    if price < 10_000:
        return 200
    return 100
...
def main(start_now=False, manual=False):
    ...
    print(f"  💼 発注: {mode_str}  候補{MIN_PRICE:,}〜{MAX_PRICE_B:,}円 / "
          f"株数 〜5千:{DEFAULT_SHARES} 5千〜1万:200 1万〜2万:100\n")
