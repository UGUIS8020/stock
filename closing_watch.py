...
MAX_PRICE_B = 20_000  # 2026-09-03新設(¥10,000)→2026-09-05に¥20,000へ引き上げ（ユーザー判断・
                    # 直近の戦略B成績が良好だったため）。戦略Bには候補の価格上限が無く、
                    # HIGH_PRICE_CAP_AMOUNTも単元株100株の下限があるため高値株で無力
                    # （ファストリ68,000円→100株680万円が発生したのが¥10,000導入の経緯）。
                    # ¥20,000×100株の建玉は約¥200万（HIGH_PRICE_CAP_AMOUNTの下限100株が効くため
                    # ¥100万→¥200万の範囲）。戦略A(MAX_PRICE_A=10,000)・AS より上限は緩い。
