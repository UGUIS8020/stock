...
条件（2026-08-10 大サンプル・ウォークフォワード検証済み、2年データ）:
    score       : 1.5 <= score < 3.0     （calc_score_an と同一の計算式）
    ratio       : ratio >= 1.4
    today_rise  : -2.0% <= 当日の寄り引け値幅 <= +9.1%   （2026-08-10: -1.2%→-2.0%に緩和）
    rb_score    : calc_rebound_score(hist) >= 5.4
    consec_drop : 連続下落日数 >= 1
    TP/SL       : +1.5% / -5.9%（エントリー翌営業日の寄り付きで買う想定）
    検証結果    : 訓練n=263 勝率63.1% avg+0.414% / 検証n=264 勝率66.7% avg+0.313%
                  （月別・銘柄別の偏りなしを確認済み。50/60/70/80%の全分割点で
                  　訓練・検証とも旧条件(-1.2%)を上回ることを確認。GA独立探索でも
                  　rise_lo≈-2.1の解が2位(out_sharpe 3.958)に自然に出現し裏付け済み）

    2026-08-10のRISE_MIN変更に伴う検証メモ:
...
SCORE_MAX   = 3.0
RATIO_MIN   = 1.4
RISE_MIN    = -2.0   # 2026-08-10: -1.2→-2.0（件数1.26→1.76件/日に増加、訓練・検証とも改善確認済み）
RISE_MAX    = 9.1
RB_MIN      = 5.4
CD_MIN      = 1
