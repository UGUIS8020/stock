...
def build_market_conditions():
    ...
    daily_market_stats.csv があればそちらを優先使用。
    """
...
def scan_all_codes(market_cond, drop_lo=-99, drop_hi=-4.0):
    """drop_lo <= today_rise <= drop_hi の銘柄を SQLite から抽出"""
    trades = []
...
def summarize(sub, label, tp=None, sl=None):
    ...
    rets = sub["ret_oc"].values if tp is None else sim_tp_sl(sub, tp, sl)
    wr   = (rets > 0).mean() * 100
    avg  = rets.mean()
    cum  = rets.sum()
    mark = " ★" if avg > 0 else ""
...
def main():
    ...
    print("\n全銘柄スキャン中（-4%以下）...")
    df4 = scan_all_codes(market_cond, drop_lo=-99, drop_hi=-4.0)
    ...
    for cond_label, cond_val in [
        ("全地合い", "ALL"), ("STRONG", "STRONG"),
