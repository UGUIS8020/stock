...
MAX_POSITIONS    = 7   # 戦略A（2026-08-12: 5→7。STRONG日の実候補は平均65件/日・90%ileで
                        #   123件と桁違いに豊富で、上限5は98.4%の日で機会損失が発生していた。
                        #   rank別累積検証で7件まで質を落とさず利益が伸びることを確認
                        #   （avg+0.649%→+0.673%、1日平均損益+9,733円→+14,125円）。
                        #   建玉サイズは変更せず（半分サイズへの縮小は6〜10位の質低下により
                        #   むしろ損、と検証済みのため見送り）。資金拘束は5×30万=150万円→
                        #   7×30万=210万円に増加（他戦略との合計は800万円に対しやや超過するが
                        #   許容範囲と判断）
...
MAX_POSITIONS_AS = 10  # 戦略AS（2026-08-12: 5→10。STRONG日は1日25〜500件超と候補が
                        #   桁違いに多く、rank別累積検証で6〜10位の候補も1〜5位と同等以上の
                        #   質(avg+0.68%→+0.77%へ改善、質低下なし)と確認済み。建玉サイズ
                        #   (AS_DEFAULT_SHARES/AS_MAX_ORDER_AMOUNT)は変更せず、資金拘束は
...
DEFAULT_SHARES   = 200      # 高値株のデフォルト株数（戦略B/Dと同水準）
CHEAP_THRESHOLD  = 1_500   # この価格未満は最大株数モード（戦略B/Dと同水準。2026-08-06:
                            # 大サンプル検証で価格帯による収益差が確認できなかったため、
                            # 安い株への傾斜配分(50万円)は撤回しB/Dと完全統一）
MAX_ORDER_AMOUNT = 300_000  # 安い株の1発注上限額（2026-08-06: 50万→30万円。戦略B/Dと統一し、
