...
DEFAULT_SHARES   = 200      # 高値株のデフォルト株数（2026-08-06: MAX_ORDER_AMOUNTのみ20万→50万円に
                            # 引き上げ、株数は200のまま。安い株側に厚く配分しつつ資金所要額を抑制）
CHEAP_THRESHOLD  = 1_500   # この価格未満は最大株数モード（2026-08-06: 1,000→1,500円。200株との
                            # 境界では断層が残る（1,499円→300株/1,501円→200株）が許容）
