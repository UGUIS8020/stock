# -*- coding: utf-8 -*-
"""
scan_morning_macro_scan.py - マクロスキャナー（大型株マクロ連動・表示専用）

scan_morning.py から分割。今後ほぼ変更不要な安定ロジック。
銅/原油/ドル円/半導体ETFの動きから恩恵を受ける大型株を提示する。
※自動発注なし。戦略D（日中シグナル daytime.py）とは別物。
"""
...
def scan_strategy_d(macro, condition, tachibana_prices=None):
    ...
    print(f"\n{'='*60}")
    print(f"【マクロスキャナー】大型株マクロ連動候補（スコアリング付き・表示専用）")
    print(f"  参考: 銅/原油/ドル円/半導体の動きから恩恵銘柄を提示")
    print(f"{'='*60}")

    if condition == "PANIC":
        print(f"  🚨 地合いPANIC - マクロスキャナーも全見送り推奨")
        return
    ...
    for sector, val, val_prev, lag_note in triggered:
        ...
        # 銀行セクター（金利トリガー）はバックテストでスコアフィルターが逆効果だったため不使用
        # （optimize/optimize_f.py 検証: score>=7.5適用時 avg-0.063% / 未適用時 avg+0.075%〜、N=2455）
        is_bank_sector = sector["trigger_key"] == "rate"
